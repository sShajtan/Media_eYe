...
const PutOnMarketplace = (props) => {
  ...
  const [quantityValue, setQuantityValue] = useState(product[0].quantity);
  const [putForSale, setPutForSale] = useState(0);
  ...
  const putForSaleChange = (e) => {
    if (
      (Number(e.target.value) <= product[0].quantity) &
        (Number(e.target.value) >= 0) ||
      e.target.value == null
    ) {
      setQuantityValue(quantityValue + putForSale - e.target.value);
      setPutForSale(Number(e.target.value));
    }
  };

  useEffect(() => {}, [putForSale]);
  ...
  return (
    <div className="put_on_marketplace_create">
      ...
      <PopupImage
        ...
        img={product[0].fullImage}
      />
      <div className="container">
        <div className="product_main">
          <div className="product_left">
            <div className="product_image" onClick={togglePopupImage}>
              <img src={product[0].fullImage[0]} alt="product"></img>
            </div>
            ...
            <div className="">
              ...
              <div className="product_auction_details">
                <div className="product_auction_details_row">
                  ...
                  <span>{product[0].info.type}</span>
                </div>
                <div className="product_auction_details_row">
                  <span>Label</span>
                  <span>{product[0].info.label}</span>
                </div>
                <div className="product_auction_details_row">
                  <span>Creator address</span>
                  <span className="adress">{product[0].info.creator}</span>
                </div>
                <div className="product_auction_details_row">
                  <span>NFT address</span>
                  <span>{product[0].info.adress}</span>
                </div>
                <div className="product_auction_details_row">
                  <span>NFT Token ID</span>
                  <span>{product[0].info.token_id}</span>
                </div>
                <div className="product_auction_details_row">
                  <span>The number of tokens created</span>
                  <span>{product[0].info.number}</span>
                </div>
              </div>
            </div>
          </div>
          <div className="product_right">
            ...
            <div className="collection_block collection_block_buttons collection_block_blockchain">
              ...
              >
                {product[0].quantity && product[0].creator ? (
                  <div className="quantity_block">
                    <div>
                      <label>Quantity</label>
                      <input type="number" value={quantityValue} disabled />
                    </div>
                    <div>
                      <label>Put for sale</label>
                      <input
                        type="number"
                        max={product[0].quantity}
                        min={0}
                        placeholder="Enter quantity for sale"
                        value={putForSale}
